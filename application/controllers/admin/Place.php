...
class Place extends CI_Controller {
	...
	{	
		...
		//글목록
		$data['list'] = $this->place_mdl->get_place_list();

		$this->load->view('admin/layout/header.php');
		$this->load->view('admin/place_list.php', $data);
	}

	//작성글 저장
	public function apply_place()
	{	
	...
	public function upload_image() {
        if ($_FILES['file']['name']) {
            $config['upload_path'] =  "images/place/";
            ...
            if (!$this->upload->do_upload('file')) {
                ...
                $file_path = base_url("images/place/".$data['file_name']);
                echo $file_path;
            }
        }
    }
	...
	{	
        ...
        $this->load->view('admin/layout/header.php');
        $this->load->view('admin/place_apply.php', $data);
	}
	...
	{	
        ...
        $data['info'] = $this->place_mdl->get_place_info($id);

        if (!empty($data)) {
            
            $this->load->view('admin/layout/header.php');
            $this->load->view('admin/place_modify.php', $data);
	...
	public function regi_place() {
        ...
        if ($this->input->is_ajax_request()) 
        {
            ...
            $place = $this->place_mdl->get_place_info($id);
            ...
            if(!empty($id))
            {
                ...
                $res = $this->place_mdl->update_place($id, $data);
            ...
            {
                $res = $this->place_mdl->insert_place($data);
